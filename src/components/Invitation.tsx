...
export default function Invitation({ guestName, initialVisits }: Props) {
  ...
  const showToast = useCallback((message: string) => {
    ...
    toastTimer.current = setTimeout(() => setToast((p) => ({ ...p, show: false })), 2600);
  }, []);

  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const value = params.get("to") || params.get("n") || params.get("guest") || params.get("kepada");
  ...
  useEffect(() => {
    const saved = Number.parseInt(localStorage.getItem("invitation-visits") || "0", 10);
  ...
  const openInvitation = async () => {
    ...
    void playMusic();
    const nextVisits = Number.parseInt(localStorage.getItem("invitation-visits") || "0", 10) + 1;
    ...
    <>
      ...
      >
            ...
            <Reveal variant="left">
              <figure className="relative flex w-full flex-col items-center">
                ...
                <div
                  ...
                  style={{ boxShadow: "18px 14px 40px -12px rgba(107,125,94,.45)" }}
                >
                  <img
        ...
        <footer
          ...
          style={{ background: "linear-gradient(135deg, var(--sage), var(--sage-700))" }}
        >
